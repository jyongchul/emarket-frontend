...
# -*- coding: utf-8 -*-
"""
E-MARKET Project - TEST Email Script
Sends TEST email to jyongchul@gmail.com for verification
"""
...
RECIPIENT_EMAIL = "jyongchul@gmail.com"  # TEST: Send to self
RECIPIENT_NAME = "이종철 (테스트)"  # TEST
...
<head>
    ...
    <title>E-MARKET 프로젝트 진행 상황 보고 [TEST]</title>
    <style>
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f5f5f5;">
        <tr>
            <td align="center" style="padding: 20px 0;">
                <!-- TEST BANNER -->
                <div style="max-width: 1000px; background-color: #fef3c7; border: 2px solid #f59e0b; border-radius: 8px; padding: 15px; margin-bottom: 20px; text-align: center;">
                    <strong style="color: #92400e; font-size: 16px;">⚠️ 테스트 이메일 - 실제 고객에게 발송되지 않음</strong>
                </div>
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" class="container" style="max-width: 1000px; width: 100%; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
                    ...
                    <tr>
                        <td class="header" style="background: linear-gradient(135deg, #059669 0%, #10b981 100%); padding: 30px; text-align: center;">
                            ...
                            <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);">E-MARKET 프로젝트 진행 상황 [TEST]</h1>
                    ...
                    <tr>
                        <td class="content" style="padding: 40px;">
                            ...
                            <p style="margin: 0 0 25px 0; color: #333333; font-size: 16px; line-height: 1.8; word-break: keep-all;">
                                안녕하세요, <strong style="color: #059669;">임수진</strong> 대표님.
                            </p>
...
def send_email():
    """
    Send TEST HTML email with embedded logo
    """
    try:
        print("\n" + "="*70)
        print("E-MARKET 프로젝트 진행 보고 TEST 이메일 발송 시작")
        print("="*70)
        print(f"⚠️  TEST MODE - 실제 고객에게 발송되지 않음")
        print(f"시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"발신자: {SENDER_NAME} <{SENDER_EMAIL}>")
        print(f"수신자 (TEST): {RECIPIENT_NAME} ({RECIPIENT_EMAIL})")
        print(f"참조: {CC_EMAIL}")
        print("-"*70)

        # Create message
        msg = MIMEMultipart('related')
        msg['Subject'] = f"[TEST] E-MARKET 프로젝트 진행 상황 보고 - {datetime.now().strftime('%Y년 %m월 %d일')}"
        ...
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT) as server:
            ...
            # Send to test recipient and CC
            recipients = [RECIPIENT_EMAIL, CC_EMAIL]
            server.send_message(msg, to_addrs=recipients)

        print("="*70)
        print("✅ TEST 이메일 전송 성공!")
        print("="*70)
        print(f"수신자 (TEST): {RECIPIENT_NAME} ({RECIPIENT_EMAIL}) ✓")
        print(f"참조: {CC_EMAIL} ✓")
        print("\n다음 단계:")
        print("1. 이메일을 확인하세요")
        print("2. 레이아웃, 내용, 로고가 올바른지 확인하세요")
        print("3. 문제가 없으면 실제 고객용 스크립트를 실행하세요:")
        print("   python3 /mnt/c/EMARKET/email-automation/send_daily_email.py")
        print("="*70 + "\n")
